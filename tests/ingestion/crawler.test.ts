...
describe('crawlJsFiles', () => {
  it('collects supported extensions and normalizes paths', async () => {
    ...
    const files = [
      ...
      'other.js',
      'notes.txt',
      path.join('nested', 'file.mjs'),
      path.join('nested', 'sub', 'file.tsx'),
    ];
    ...
    const expected = [
      ...
      path.join('nested', 'file.mjs'),
      path.join('nested', 'sub', 'file.tsx'),
    ].map((p) => normalize(path.join(tempDir, p)));
...
describe('crawlFiles', () => {
  it('supports language-based extension sets', async () => {
    ...
    const expected = ['main.go', 'handler.js', 'component.tsx'].map((p) =>
      normalize(path.join(tempDir, p))
    );
